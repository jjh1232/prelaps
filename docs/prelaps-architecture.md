...
#### 도구 (`/<도구>/`) — 새로 만드는 것은 전부 이 구조

**언어판은 예외 없이 `/<도구>/<언어>/` 아래 둔다. 루트에 특정 언어를 놓지 않는다.**

```
prelaps.com/도구/          ← 통로. 언어별로 302. 색인 대상 아님
prelaps.com/도구/ko/       ← 한국어 실물
prelaps.com/도구/en/       ← 영어 실물
prelaps.com/도구/ja/       ← 일본어 실물
```

- 루트는 sitemap 에서 뺀다. canonical 을 달지 않는다. hreflang 이 가리키지 않는다.
- 언어판은 각자 자기 자신을 canonical 로 하고, 서로 전체 hreflang 목록을 갖는다.
- 루트의 302 규칙은 허브와 같다 — 아래 표 그대로.

**한 언어를 루트에 두면 나중에 못 고친다.** 그 주소가 곧 그 언어의 canonical 이 되어
sitemap·hreflang·외부 링크·색인이 전부 거기 걸린다. 그 뒤에 언어 분기를 붙이려면
canonical 주소를 리다이렉트로 만들어야 하는데, 그건 하면 안 되는 일이다.
그래서 분기를 붙일 자리를 처음부터 비워 둔다 — 도구 하나 만들 때 폴더 이름 하나 차이다.

> **mojibake 는 이 규칙을 따르지 않는다 — 알고 두는 것이다.**
>
> 한국어가 `/mojibake/` 루트에 있어서, 그 주소는 누가 오든 한국어다.
> 서브도메인 시절 색인 경로를 유지하려고 그렇게 뒀다(`tools.ts` 에도 기록).
>
> **2026-08-19: 옮기지 않기로 결정.** 허브 `/` 에 언어 분기를 붙이면서 같이 볼지
> 검토했고, 미루기로 했다. 8/18 에 서브도메인에서 옮긴 직후라 주소를 또 바꾸면
> 301 이 겹치고(`mojibake.prelaps.com` → `/mojibake/` → `/mojibake/ko/`),
> 그 사이 실이익이 작다 — 영어·일본어 검색 유입은 구글이 hreflang 을 보고
> `/en/`·`/ja/` 로 직접 보내고, 허브 카드 링크도 이미 언어별로 갈린다.
> 한국어를 보게 되는 사람은 `/mojibake/` 를 직접 치거나 공유받은 경우뿐이다.
>
> **다시 볼 시점:** 도구를 크게 개편할 때, 또는 영어·일본어 유입이 붙어
> `/mojibake/` 직접 진입이 늘어날 때. 트래픽이 적을수록 옮기는 값이 싸다.
>
> 새 도구는 이걸 따라하지 말 것. 처음부터 언어 폴더를 두면 이 고민이 없다.

#### 허브 루트 (`/`)

2026-08-19 부터 `Accept-Language` 로 가른다. (prelaps-router)

원래 이 문서는 자동 리다이렉트를 비권장으로 적어 두었다. 이유는 "구글봇이 늘
같은 언어로만 들어와 다른 언어가 색인되지 않는 사고" 였다. 그 위험은 구글이
`/` 를 거쳐야만 다른 언어를 발견할 수 있을 때 성립하는데, 지금 허브는

- `/ko`·`/en`·`/ja` 가 sitemap 에 각각 직접 올라가 있고,
- 세 페이지가 hreflang 으로 서로를 전부 가리키며,
- `/` 는 sitemap 에서 아예 빼 두었다 (astro.config.mjs 의 `filter`).

이 세 가지가 갖춰져 있어 전제가 깨졌다. 되돌린 계기는 Web Analytics 에서
미국 방문 3건이 전부 `/ko` 로 찍힌 것 — 영어권 방문자가 한국어 화면을 보고 있었다.

**바꿀 때 같이 지켜야 하는 것** (하나라도 빠지면 조용히 망가진다):

| | 왜 |
|---|---|
| `302`, 절대 `301` 아님 | 301 은 브라우저가 영구 캐시한다. 한 번 `/en` 을 맞으면 주소창에 `prelaps.com` 을 쳐도 다시는 한국어로 못 간다 |
| `Cache-Control: no-store` | Cloudflare 엣지는 `Accept-Encoding` 외의 `Vary` 를 무시한다. `Vary` 만 믿으면 첫 방문자의 언어가 모두에게 나간다 |
| `/ko`·`/en`·`/ja` 자체는 무손질 | 그 주소들까지 헤더로 덮으면 언어 전환 UI 가 죽는다 |
| 폴백은 `DEFAULT_LOCALE`(ko) 이 아니라 `XDEFAULT_LOCALE`(en) | 헤더를 못 맞춘 방문자가 곧 x-default 가 답하는 대상이다. hreflang 이 x-default=en 이라고 말하면서 실제로는 ko 로 보내면 신호가 어긋난다 |

`public/_redirects` 의 `/ → /ko` 는 지우지 않는다. `prelaps-home.pages.dev` 로
직접 들어온 요청용 폴백이다 — 그쪽은 Worker 를 거치지 않는다.
...
□ 언어판을 전부 /<도구>/<언어>/ 아래 둔다. 루트에 특정 언어를 놓지 않는다  ← §5
    · /<도구>/     통로. sitemap 제외, canonical 없음, hreflang 이 안 가리킴
    · /<도구>/ko/  /<도구>/en/  /<도구>/ja/   각자 자기 자신 canonical
    · 한 언어를 루트에 두면 그 주소가 canonical 이 되어 나중에 언어 분기를 못 붙인다
      (mojibake 가 그 상태다 — 따라하지 말 것)
□ 도구 Worker 에서 /<도구>/ 를 Accept-Language 로 302     ← §5 의 조건 표 그대로
    · 302 (301 아님) / Cache-Control: no-store / 폴백은 영어
    · /<도구>/ko/ 등 언어판 자체는 손대지 않는다
